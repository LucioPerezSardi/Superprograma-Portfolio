...
from PyQt6.QtWidgets import (
    ...
    QStyledItemDelegate, QStyleFactory, QCheckBox  # Agregado QCheckBox
)
...
# Clase para la pestaña de Análisis
class AnalysisTab(QWidget):
            ...
            for tipo in ["bonos", "acciones", "cedears","etfs", "cripto", "fci"]:
                ...
                for _, row in enumerate(tipo_rows):
                    ...
                    if symbol in self.portfolio_symbols:
                        symbol_item.setBackground(QColor("#FFA500"))  # Naranja
                ...
                # Ordenar inicialmente por símbolo
    ...
    def update_combo_color(self, combo):
        ...
        if text == "Muy Alta":
            ...
            color = QColor(173, 255, 47)  # Verde claro
        elif text == "Muy Baja":
            color = QColor(0, 128, 0)  # Verde oscuro
        else:
    ...
    def add_item(self, tipo, symbol, desc):
        ...
        self.update_combo_color(revision_combo)
        revision_combo.currentIndexChanged.connect(
            lambda: self.update_combo_color(revision_combo)
        )
        revision_combo.currentIndexChanged.connect(
            lambda _, r=row_position, t=tipo: self.on_revision_changed(r, t)
        )
        ...
        self.save_data()
        self.sort_tables()  # Ordenar después de agregar nuevo ítem
        
        getattr(self, f"symbol_input_{tipo}").clear()
        getattr(self, f"desc_input_{tipo}").clear()
    ...
    def delete_selected(self, tipo):
        ...
        if not selected_rows:
            QMessageBox.warning(self, "Nada seleccionado", "Por favor seleccione al menos una fila para eliminar")
            return
        
        reply = QMessageBox.question(self, "Confirmar eliminación", 
                                    f"¿Está seguro que desea eliminar {len(selected_rows)} fila(s)?",
        ...
        self.save_data()
        self.sort_tables()  # Ordenar después de eliminar   
    ...
    def update_all(self):
        ...
        self.save_data()
        QMessageBox.information(self, "Actualizado", "Todos los datos han sido guardados")
        self.sort_tables()  # Ordenar después de actualizar
    
    def sort_table_data(self, table):
        """Extrae los datos de la tabla y los ordena según los criterios"""
        revision_order = {"Muy Baja": 0, "Baja": 1, "Indeciso": 2, "Alta": 3, "Muy Alta": 4}
        data = []
        ...
        for row in range(table.rowCount()):
            ...
            desc_item = table.item(row, 1)
            revision_combo = table.cellWidget(row, 2)
            ultima_item = table.item(row, 3)
            comentario_item = table.item(row, 4)
            chart_item = table.item(row, 5)
            
            symbol = symbol_item.text() if symbol_item else ""
            desc = desc_item.text() if desc_item else ""
            revision = revision_combo.currentText() if revision_combo else ""
            ultima_revision = ultima_item.text() if ultima_item else ""
            comentario = comentario_item.text() if comentario_item else ""
            in_portfolio = symbol in self.portfolio_symbols
            ...
            data.append({
                ...
                'desc': desc,
                'revision': revision,
                'revision_value': revision_order.get(revision, 5),  # 5 para revisiones desconocidas
                'ultima_revision': ultima_revision,
                'comentario': comentario,
        ...
        if self.current_sort_criteria == "Revision":
            # Primero: elementos en portfolio (True > False)
            # Segundo: valor de revisión (ascendente)
            # Tercero: símbolo (ascendente)
            data.sort(key=lambda x: (
                not x['in_portfolio'],  # Portfolio primero (True=1, False=0 -> not True=0, not False=1)
                x['revision_value'],
                x['symbol']
            ))
        else:  # Ordenar por símbolo
            # Primero: elementos en portfolio (True > False)
            # Segundo: símbolo (ascendente)
            data.sort(key=lambda x: (
                not x['in_portfolio'],
                x['symbol']
            ))
    ...
    def apply_sorted_data(self, table, sorted_data):
        """Aplica los datos ordenados a la tabla, mostrando mensaje mientras procesa"""
        # Mostrar mensaje de ordenamiento
        sorting_msg = QLabel("Ordenando datos, por favor espere...")
        sorting_msg.setAlignment(Qt.AlignmentFlag.AlignCenter)
        table.setRowCount(0)
        table.setRowCount(1)
        table.setSpan(0, 0, 1, table.columnCount())
        table.setCellWidget(0, 0, sorting_msg)
        ...
        for item in sorted_data:
            ...
            index = revision_combo.findText(item['revision'], Qt.MatchFlag.MatchFixedString)
            if index >= 0:
                revision_combo.setCurrentIndex(index)
            ...
            revision_combo.currentIndexChanged.connect(
                lambda _, r=row_position, t=self.get_tipo_from_table(table): self.on_revision_changed(r, t)
            )
            
            ultima_item = QTableWidgetItem(item['ultima_revision'])
            ultima_item.setFlags(Qt.ItemFlag.ItemIsEnabled | Qt.ItemFlag.ItemIsSelectable)
            
            comentario_item = QTableWidgetItem(item['comentario'])
            comentario_item.setFlags(Qt.ItemFlag.ItemIsEnabled | Qt.ItemFlag.ItemIsEditable | Qt.ItemFlag.ItemIsSelectable)
    ...
    def sort_tables(self):
        """Ordena todas las tablas según el criterio seleccionado"""
        tipos = ["bonos", "acciones", "cedears", "etfs" ,"cripto", "fci"]
        ...
        for tipo in tipos:
            table = getattr(self, f"table_{tipo}")
            sorted_data = self.sort_table_data(table)
            self.apply_sorted_data(table, sorted_data)
    
    def get_tipo_from_table(self, table):
        for tipo in ["bonos", "acciones", "cedears", "etfs" , "cripto", "fci"]:
            if table == getattr(self, f"table_{tipo}"):
                return tipo
        return ""
    ...
    def on_cell_clicked(self, row, column, tab, table):
        if column == 5:
            ...
            tipo = ""
            for t in ["bonos", "acciones", "cedears", "etfs" , "cripto", "fci"]:
                if table == getattr(self, f"table_{t}"):
                    tipo = t
                    break
...
class PortfolioAppQt(QMainWindow):
    ...
    def load_finished_operations(self):
        ...
        for row in journal_data:
            ...
            simbolo = row['Simbolo']
            detalle = row['Detalle']
            cantidad = float(row['Cantidad'].replace(',', '.'))
            precio = float(row['Precio'].replace(',', '.'))
            rendimiento = float(row['Rendimiento'].replace(',', '.'))
            total_descuentos = float(row['Total_Descuentos'].replace(',', '.'))
            ...
            if tipo_op == "Compra":
                if simbolo not in compras_pendientes_hist:
                    compras_pendientes_hist[simbolo] = deque()
                compras_pendientes_hist[simbolo].append({
                    'cantidad': cantidad,
                    'precio': precio,
                    'fecha': fecha,
            ...
            elif tipo_op == "Rendimiento" and tipo in ["Acciones AR", "CEDEARs", "Bonos AR", "Criptomonedas", "ETFs", "FCIs AR"]:
                if simbolo not in rendimientos:
                    rendimientos[simbolo] = deque()
                rendimientos[simbolo].append({
                    'fecha': fecha,
                    'valor': rendimiento,
                    'descuentos': total_descuentos
                })
            ...
            for rend in rends:
                ...
                for compra in compras_pendientes_hist[simbolo]:
                    if compra['fecha'] <= fecha_rend:
                        tenencia_total += compra['cantidad']
        ...
        for venta in ventas:
            ...
            while cantidad_restante > 0 and compras_pendientes_hist[simbolo]:
                ...
                compra_descuentos = compra['descuentos']
                compra_rendimiento = compra['rendimiento']
                
                if compra_cantidad <= cantidad_restante:
                    costo_parcial = compra_cantidad * compra_precio
                    costo_total += costo_parcial
                    total_compra_discounts += compra_descuentos
                    total_rendimiento += compra_rendimiento
                    cantidad_restante -= compra_cantidad
                    compras_usadas.append(compras_pendientes_hist[simbolo].popleft())
                else:
                    proporcion = cantidad_restante / compra_cantidad
                    costo_parcial = cantidad_restante * compra_precio
                    costo_total += costo_parcial
                    descuento_proporcional = compra_descuentos * proporcion
                    total_compra_discounts += descuento_proporcional
                    rendimiento_proporcional = compra_rendimiento * proporcion
                    total_rendimiento += rendimiento_proporcional
                    compras_pendientes_hist[simbolo][0] = {
                        'cantidad': compra_cantidad - cantidad_restante,
                        'precio': compra_precio,
                        'fecha': compra_fecha,
                        'descuentos': compra_descuentos - descuento_proporcional,
                        'rendimiento': compra_rendimiento - rendimiento_proporcional
                    }
                    cantidad_restante = 0
            
            if cantidad_restante == 0:
                precio_compra_prom = costo_total / cantidad_vendida
                diferencia_valor = (venta['precio_venta'] - precio_compra_prom) * cantidad_vendida
                total_discounts = venta['descuentos_venta'] + total_compra_discounts
                resultado = diferencia_valor + total_rendimiento - total_discounts
                ...
                if from_date and to_date and fecha_venta:
                    if fecha_venta < from_date or fecha_venta > to_date:
                        continue
                ...
                finished_ops.append({
                    ...
                    'simbolo': venta['simbolo'],
                    'cantidad': cantidad_vendida,
                    'precio_compra': precio_compra_prom,
                    'precio_venta': venta['precio_venta'],
